-- ============================================================
-- FIX CHEF ORDER ACCEPTANCE PIPELINE
-- ============================================================
-- The chef cannot accept orders because:
-- 1. The update_order_status_sm function doesn't exist
-- 2. RLS policies may need adjustment for chef role
-- ============================================================

-- ─── FIX 1: Create update_order_status_sm function ─────────
...
-- ─── FIX 2: Ensure chef can update restaurant_orders ───────
-- The original policy "Staff update" uses is_staff() which includes chef
-- But let's make it explicit for clarity
DROP POLICY IF EXISTS "Staff update" ON restaurant_orders;
CREATE POLICY "Staff update orders"
  ON restaurant_orders FOR UPDATE
  USING (is_waiter() OR is_chef() OR is_manager() OR is_admin());

-- ─── FIX 3: Ensure chef can INSERT order_events ───────────
-- Chef needs to log status changes
DROP POLICY IF EXISTS "Staff create" ON order_events;
CREATE POLICY "Staff create order events"
  ON order_events FOR INSERT
  WITH CHECK (is_waiter() OR is_chef() OR is_manager() OR is_admin());

-- ─── FIX 4: Ensure chef can SELECT order_events ───────────
DROP POLICY IF EXISTS "Staff view" ON order_events;
...
COMMENT ON POLICY "Staff update orders" ON restaurant_orders IS 'Allows waiters, chefs, managers, and admins to update order status';
