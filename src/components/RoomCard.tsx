...
// Room image carousels by type — 3+ images each, auto-scrolling
...
export function RoomCard({ room, onBook }: RoomCardProps) {
  const images = roomCarouselImages[room.room_type] || roomCarouselImages.SINGLE;
    ...
    <article className="group card-warm overflow-hidden">
      ...
      <div className="relative h-52 bg-stone-warm/30 overflow-hidden">
        ...
        {images.length > 1 && (
          <>
            <button
              onClick={(e) => { e.stopPropagation(); goImg((imgIdx - 1 + images.length) % images.length); }}
              className="absolute left-1.5 top-1/2 -translate-y-1/2 w-6 h-6 rounded-full bg-black/30 backdrop-blur-sm flex items-center justify-center text-white/80 opacity-0 group-hover:opacity-100 transition-opacity z-10"
            >
              <ChevronLeft className="h-3 w-3" />
            </button>
            <button
              onClick={(e) => { e.stopPropagation(); goImg((imgIdx + 1) % images.length); }}
              className="absolute right-1.5 top-1/2 -translate-y-1/2 w-6 h-6 rounded-full bg-black/30 backdrop-blur-sm flex items-center justify-center text-white/80 opacity-0 group-hover:opacity-100 transition-opacity z-10"
            >
            ...
            <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-1.5 z-10">
              {images.map((_, i) => (
                <button
                  ...
                  className={cn("w-1.5 h-1.5 rounded-full transition-all duration-300", i === imgIdx ? "bg-white w-4" : "bg-white/50")}
                />
              ))}
            </div>
          </>
